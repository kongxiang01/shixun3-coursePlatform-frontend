  ...
  <div class="outline-container">
    <div class="header">
      ...
      <template v-if="userInfo.type === '1'" >
        <el-button @click="dialogVisible = true" style="margin-right: 20px">上传大纲</el-button>
      </template>
      <el-dialog
          title="上传大纲"
...
import {outlinePreviewService, outlineUploadService} from "@/api/course.js";
...
const submitUploadForm = async () => {
  ...
  try {
    if (uploadFormData.value.file) {
      //console.log('TeachingCalendar.vue: 11111111111111111teachingCalendarPath:');
      await outlineUploadService(uploadFormData.value.file, courseInfo.value.cid)
...
const previewTeachingCalendar = async () => {
  ...
  const res = await outlinePreviewService(courseInfo.value.cid);
